...
func (p *Plan) mounts() []string {
	m := []string{
		p.cfg.HostWorktree + ":" + WorkDir + ":rw,z",
...
// DefaultCommand is the Claude Code implementer invocation used when the
// caller doesn't override Command. Bash is disallowed at the native layer
// (§5.2 tier 3) so the agent must reach the shell through the policy-gated
// hobbes exec tool; Edit/Write on the worktree stay allowed.
func (p *Plan) DefaultCommand() []string {
	return []string{
		...
		"--allowedTools",
		"mcp__hobbes__exec,mcp__hobbes__graph_neighborhood," +
			"mcp__hobbes__who_calls,mcp__hobbes__tests_guarding,Edit,Write,Read",
	}
}
