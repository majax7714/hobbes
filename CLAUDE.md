...
**2026-08-22, later: ADR-063 — parallel implementers in waves over the contract DAG, vLLM-gated (`--parallel auto`, C-51); measured: per-unit harness overhead ~1 s, decode at 28 tok/s is 85–90 % of a unit's wall. The role-context fix is built (ADR-062** — the planner handoff is projected per unit via `planner_slice`, the Interior section is never cut; 830 pytest). **A fresh session resumes from `docs/phase4-to-45set-handoff.md`** —
