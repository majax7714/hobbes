"""Invariant records: loading, validation, verdicts, compilation (M8)."""
...
from hobbes.invariants.verdict import FAIL, PASS, SKIPPED, SOFT, UNKNOWN, judge_all
...
def forbidden_import_record(**overrides) -> dict:
    record = {
        "id": "I-1",
...
class TestLoad:
    def test_loads_a_valid_record(self, repo):
        write_record(repo, "I-1-parsers", forbidden_import_record())
    ...
    def test_statement_is_normalized_to_one_line(self, repo):
        write_record(
            repo, "I-1", forbidden_import_record(statement="a\n  b\n\n  c")
        )
        (record,) = load_all(repo)
    ...
    def test_ids_sort_numerically(self, repo):
        for n in (10, 2, 1):
            write_record(repo, f"I-{n}", forbidden_import_record(id=f"I-{n}"))
        assert [r.id for r in load_all(repo)] == ["I-1", "I-2", "I-10"]

    def test_every_problem_is_reported_at_once(self, repo):
        write_record(repo, "bad", {"id": "I-1", "compile": {"target": "nope"}})
        ...
        assert "compile.target" in problems

    def test_duplicate_ids_are_refused(self, repo):
        write_record(repo, "a", forbidden_import_record())
        write_record(repo, "b", forbidden_import_record())
        with pytest.raises(ValidationError, match="duplicate id I-1"):
            load_all(repo)

    def test_unknown_fields_are_refused(self, repo):
        write_record(repo, "I-1", forbidden_import_record(severity="high"))
        with pytest.raises(ValidationError, match="unknown field"):
            load_all(repo)

    def test_soft_record_with_a_rule_is_refused(self, repo):
        # Writing a spec and then telling the compiler to ignore it means
        # one of the two is wrong.
        write_record(
            repo,
            "I-1",
            forbidden_import_record(
                compile={
                    "target": "soft",
        ...
        with pytest.raises(ValidationError, match="soft records take no compile.rule"):
            load_all(repo)

    def test_structured_target_without_a_rule_is_refused(self, repo):
        write_record(
            repo, "I-1", forbidden_import_record(compile={"target": "semgrep"})
        )
        with pytest.raises(ValidationError, match="compile.rule is required"):
            load_all(repo)
    ...
    def test_guarded_by_must_name_real_tests(self, repo):
        write_record(
            repo, "I-1", forbidden_import_record(guarded_by=["tests/gone.py::test_x"])
        )
    ...
    def test_inferred_status_warns_but_stays_inert(self, repo):
        ...
        write_record(repo, "I-1", forbidden_import_record(status="inferred"))
        (record,) = load_all(repo)
...
class TestVerdicts:
    def test_clean_graph_passes(self, repo, graph):
        write_record(repo, "I-1", forbidden_import_record())
    ...
    def test_scope_bounds_who_the_rule_is_about(self, repo, graph):
        ...
        write_record(repo, "I-1", forbidden_import_record(scope="."))
        (verdict,) = judge_all(load_all(repo), graph)
        assert verdict.result == FAIL
    ...
    def test_prefix_patterns_match_packages_and_directories(self, repo, graph):
        ...
        record = forbidden_import_record()
        record["compile"]["rule"]["importers"] = ["app.*"]
        record["compile"]["rule"]["except"] = ["app.parser"]
    ...
    def test_soft_records_are_handed_to_a_reviewer(self, repo, graph):
        write_record(
            repo, "I-1", forbidden_import_record(compile={"target": "soft"})
        )
        (verdict,) = judge_all(load_all(repo), graph)
    ...
    def test_unconfirmed_records_are_skipped(self, repo, graph):
        write_record(repo, "I-1", forbidden_import_record(status="retired"))
        (verdict,) = judge_all(load_all(repo), graph)
    ...
    def test_worst_verdict_sorts_first(self, repo, graph):
        ...
        write_record(repo, "I-1", forbidden_import_record(id="I-1"))
        write_record(
            repo, "I-2", forbidden_import_record(id="I-2", compile={"target": "soft"})
        )
    ...
    def test_a_guard_that_vanished_is_reported(self, repo, graph):
        write_record(
            repo, "I-1", forbidden_import_record(guarded_by=["tests/t.py::test_a"])
        )
...
class TestCompile:
    ...
    def test_soft_and_unconfirmed_records_are_skipped_with_a_reason(self, repo, graph):
        write_record(repo, "I-1", forbidden_import_record(compile={"target": "soft"}))
        write_record(repo, "I-2", forbidden_import_record(id="I-2", status="retired"))
        manifest = compile_all(repo, load_all(repo), graph)
